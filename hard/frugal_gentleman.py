...
Given a list of wine dictionaries, write a function that returns the name
of the wine he will buy for the party. If given an empty list, return None.
...
def chosen_wine(wines):
    if wines:
        wines = sorted(wines, key=lambda wine: wine["price"])
        return wines[1]["name"] if len(wines) > 1 else wines[0]["name"]
    return None
